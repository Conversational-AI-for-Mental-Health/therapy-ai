...
// Ensures PYTHON_AI_URL always resolves to the /chat endpoint,
// regardless of whether the env var was set with or without the path.
...
const config: AppConfig = {
  ...
  GOOGLE_CLIENT_ID: process.env.GOOGLE_CLIENT_ID, // Optional for future social login features
  APPLE_CLIENT_ID: process.env.APPLE_CLIENT_ID,   // Optional for future social login features
};

// Startup validation — throws hard so misconfiguration is caught immediately
...
// JWT_SECRET must be set to a real value in any non-test environment.
// Crashing here is intentional — a weak secret in production is a critical vulnerability.
