...
import {
  View, Text, TouchableOpacity, StyleSheet, ScrollView, Modal, Pressable, Linking, Alert,
} from 'react-native';
...
const getWebUrl = (path: string) => {
  try {
    ...
    const basePath = apiBase.replace(/:\d+\/api.*$/, '');
    return `${basePath}:8080${path}`;
  } catch {
...
const LINKS = {
  get ourStory() { return getWebUrl('/story/our-story'); },
  get privacyPolicy() { return getWebUrl('/privacy'); },
  get contactUs() { return getWebUrl('/contact'); },
} as const;
...
export default function Sidebar({
  ...
  onSelectChat, onDeleteChat, onContactProfessional, onNavigate,
  onLoadMoreSessions, user,
}: SidebarProps) {
          ...
          <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
            ...
            <ScrollView
              ...
              onScroll={({ nativeEvent }) => {
                ...
                if (nearBottom && onLoadMoreSessions && !isLoadingMoreRef.current) {
                  isLoadingMoreRef.current = true;
                  Promise.resolve(onLoadMoreSessions()).finally(() => {
            ...
            <View style={styles.footer}>
              <TouchableOpacity
                ...
                onPress={() => { onClose(); onContactProfessional(); }}
              >
...
const styles = StyleSheet.create({
  ...
  drawer: { width: 280, backgroundColor: colors.surface, borderRightWidth: 1, borderRightColor: colors.border, position: 'absolute', left: 0, top: 0, bottom: 0 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: spacing.md, paddingTop: spacing.md, paddingBottom: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.border },
  userRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  avatar: { width: 36, height: 36, backgroundColor: colors.primary, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  ...
  newChatBtn: { margin: spacing.md, backgroundColor: colors.surface2, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, padding: 10, alignItems: 'center' },
  newChatText: { color: colors.text, fontSize: 13, fontWeight: '500' },
  sectionLabel: { paddingHorizontal: spacing.md, paddingBottom: spacing.sm, fontSize: 10, textTransform: 'uppercase', letterSpacing: 1, color: colors.textMuted },
  chatList: { flex: 1, paddingHorizontal: spacing.sm },
  chatItem: { padding: 10, borderRadius: radius.sm, marginBottom: 2 },
  ...
  footer: { borderTopWidth: 1, borderTopColor: colors.border, padding: spacing.sm, gap: 4 },
  contactBtn: { backgroundColor: colors.surface2, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, padding: 10, alignItems: 'center', marginBottom: 4 },
  contactBtnText: { color: colors.accent, fontSize: 13, fontWeight: '600' },
  footerBtn: { padding: 10, borderRadius: radius.sm },
